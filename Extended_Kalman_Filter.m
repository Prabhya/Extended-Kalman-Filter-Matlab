...
%%%%% Error Covariances %%%%%
q = 0.5*10^(-3)*eye(2);     % Process noise error covariance
...
X1_x = -10*cos(0.001*t)*3000;
X1_y =  5*sin(0.005*t)*3000;
...
for ie = 2:m
    %%%%%%%%% PROPAGATION %%%%%%%%%%
    % Their are different options over the type of propagation
    %%% Discrete propagation %%%
    % propagate with and without control input
    %xe   = phi*xe;     %+gam*u(ie-1,:)';
    %pcov = phi*pcov*phi' + gamw*Q*gamw';
    %pcov = phi*pcov*phi' + gamwn*Q*gamwn';   % choosing different error model for propagation and estimation
    
    %%% Continuous propagation %%%
    X    = ode4(@func,[t(ie-1) t(ie)],[xe;u(ie-1,:)';reshape(pcov,[],1)],A,B,G,Q);
    %X    = ode4(@func,[t(ie-1) t(ie)],[xe;u(ie-1,:)';reshape(pcov,[],1)],A,B,Gn,Q);   % choosing different error model for propagation and estimation 
    xe   = X(end,1:6)';
